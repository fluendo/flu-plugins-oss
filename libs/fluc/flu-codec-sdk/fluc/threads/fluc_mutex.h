...
#define CAPABILITY(x) \
  ANNOTATION(capability(x))

#define REQUIRES(...) \
  ANNOTATION(requires_capability(__VA_ARGS__))

#define ACQUIRE(...) \
  ANNOTATION(acquire_capability(__VA_ARGS__))

#define RELEASE(...) \
  ANNOTATION(release_capability(__VA_ARGS__))

#define TRY_ACQUIRE(...) \
  ANNOTATION(try_acquire_capability(__VA_ARGS__))

#define EXCLUDES(...) \
  ANNOTATION(locks_excluded(__VA_ARGS__))

#define RETURN_CAPABILITY(x) \
  ANNOTATION(lock_returned(x))
...
#define FLUC_UNLOCK(lock_type, lock) do {                    \
