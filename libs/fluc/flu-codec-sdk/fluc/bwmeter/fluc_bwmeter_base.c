...
fluc_bwmeter_base_init (FlucBwMeter * meter)
{
  ...
  meter->stats.raw = 0;
  meter->stats.avg = 0;
  GST_DEBUG_CATEGORY_INIT (bwmeter_debug, "bwmeter",
      0, "Fluendo bandwidth meter");
}
...
fluc_bwmeter_base_update (FlucBwMeter * meter)
{
  ...
  fluc_rmutex_lock (&meter->lock);
  elapsed = (float) (time - meter->state.time_start) /
      (float) G_TIME_SPAN_SECOND;
...
fluc_bwmeter_base_compute (FlucBwMeter * meter, gint64 time)
{
  ...
  if (elapsed >= meter->config.time_min ||
     ...
     * The measured raw value is provided to allow consumers to compute averaging
     * with any other criteria if needed.
     * We do not share the computation assigning the fall/rise factor first
    ...
    if (raw <= meter->stats.avg) {
      ...
      avg = !meter->stats.avg ? raw : raw * meter->config.avg_rise_factor +
          meter->stats.avg * ((float) 1.0 - meter->config.avg_rise_factor);
    }

    GST_DEBUG ("bwmeter compute raw=%f, avg=%f elapsed=%f bytes=%u\n", raw, avg,
        elapsed, meter->state.bytes);
