 ...
 * */
GHashTable *fludownloader_helper_downloader_parameters_new (const gchar * firstfield, ...);
...
* - proxy: G_TYPE_STRING
* */
GHashTable *fludownloader_helper_downloader_parameters_add (GHashTable * table, const gchar * firstfield, ...);

/* Initialize and retrieve a helper structure for a new download with given parameters
 * The hash table 'parameters' can be retrieved by the API above and can be NULL.
 * */
FluDownloaderHelper *fludownloader_helper_downloader_new (GHashTable* parameters);
...
/* Send a HEAD request to given url with given parameters and wait for response.
 * Returns TRUE and response header (a NULL-terminated array of strings,
 * call g_strfreev after usage) on success.
 * Parameters can be NULL.
 * Returns FALSE and a status code on failure.
