...
typedef gboolean (*FluDownloaderDataCallback) (
...
typedef void (*FluDownloaderDoneCallback) (FluDownloaderTaskOutcome outcome,
    ...
    FluDownloaderTask *task, gboolean *cancel_remaining_downloads);
...
FluDownloader *fludownloader_new (
...
FluDownloaderTask *fludownloader_new_task (FluDownloader *context,
    const gchar *url, const gchar *range, gpointer user_data, gboolean locked);
...
void fludownloader_abort_task (FluDownloaderTask *task);
...
void fludownloader_abort_all_tasks (
...
/* Get task outcome.*/
FluDownloaderTaskOutcome fludownloader_task_get_outcome (
    FluDownloaderTask *task);

/* Set the task outcome, useful to simulate errors for testing. */
void fludownloader_task_set_outcome (
...
const gchar *fludownloader_get_outcome_string (
    FluDownloaderTaskOutcome outcome);

/* Get ssl status for the given task */
FluDownloaderTaskSSLStatus fludownloader_task_get_ssl_status (
    FluDownloaderTask *task);

/* Get a text string describing the ssl error encountered */
const gchar *fludownloader_get_ssl_status_string (
    FluDownloaderTaskSSLStatus status);
 ...
 * I.e. cookies can be retrieved from 'Set-cookie' header field of previos
 * response Only tasks added after setting new cookies will use them. */
void fludownloader_set_cookies (FluDownloader *context, gchar **cookies);

/* Set downloader user-agent */
void fludownloader_set_user_agent (
    FluDownloader *context, const gchar *user_agent);
