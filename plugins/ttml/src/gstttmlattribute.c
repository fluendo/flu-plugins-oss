    ...
    const gchar * expr)
{
  ...
  if (state->time_base == GST_TTML_TIME_BASE_CLOCK) {
    ...
    if (tmp > 0) {
      read_timeinfo.tm_sec = tmp % 60;
      tmp /= 60;
    }
    if (tmp > 0) {
      read_timeinfo.tm_min = tmp % 60;
      tmp /= 60;
    }
    if (tmp > 0) {
      read_timeinfo.tm_hour = tmp % 24;
      tmp /= 24;
    }
    ...
    const gchar ** end)
{
  ...
  if (end) {
    while (expr[n] != '\0' && g_ascii_isspace (expr[n]))
      n++;
    *end = expr + n;
  }
  ...
  switch (length->unit) {
    case GST_TTML_LENGTH_UNIT_CELLS:
      if (!state || state->frame_width == 0)
        ...
        return;
      if (direction == 0) {
        length->f = length->f * state->frame_width / state->cell_resolution_x;
      } else {
        length->f = length->f * state->frame_height / state->cell_resolution_y;
      }
      ...
      if (type == GST_TTML_ATTR_ORIGIN || type == GST_TTML_ATTR_EXTENT) {
        ...
        if (prev_attr) {
          ...
          if (!state || state->frame_width == 0)
            ...
            return;
          parent_length =
              direction == 0 ? state->frame_width : state->frame_height;
        }
  ...
  switch (attr->type) {
    ...
    case GST_TTML_ATTR_DISPLAY_ALIGN:
      attr->value.display_align =
          gst_ttml_utils_enum_parse (value, DisplayAlign);
    ...
    case GST_TTML_ATTR_SHOW_BACKGROUND:
      attr->value.show_background =
          gst_ttml_utils_enum_parse (value, ShowBackground);
    ...
    case GST_TTML_ATTR_SMPTE_IMAGETYPE:
      attr->value.smpte_image_type =
          gst_ttml_utils_enum_parse (value, SMPTEImageType);
    ...
    case GST_TTML_ATTR_SMPTE_ENCODING:
      attr->value.smpte_encoding =
          gst_ttml_utils_enum_parse (value, SMPTEEncoding);
