...
const GstTTMLToken GstTTMLUtilsTokensWrapOptionInternal[] = {
  {"wrap", GST_TTML_WRAP_OPTION_YES},
  {"noWrap", GST_TTML_WRAP_OPTION_NO},
...
{
  while (list->name != NULL && !gst_ttml_utils_attr_value_is (name, list->name))
    list++;
  return list->val;
}
...
void
gst_ttml_utils_memdump_buffer (GObject * object,
...
gst_ttml_display_align_spec_get_type (void)
{
  ...
  if (!gst_display_text_align_spec_type) {
    gst_display_text_align_spec_type =
        g_enum_register_static ("GstTTMLDisplayAlignSpec",
        display_align_spec_types);
  }
